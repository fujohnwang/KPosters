<script>
    ...
    import PayedTemplate from "./templates/PayedTemplate.svelte";

    import {content, signature} from "./store";

    let freeTemplates = [Template1, Template2, Template3]
    let templates = [...freeTemplates]

    let currentPosterTemplateIndex = 0;
    $: currentPosterTemplate = templates[currentPosterTemplateIndex]
    $: {
        if ($authenticated) {
            templates = [...templates, PayedTemplate]
        }else{
            templates = templates.splice(templates.indexOf(templates), 1)
        }
    }

    function previous() {
        currentPosterTemplateIndex = currentPosterTemplateIndex - 1 < 0 ? templates.length - 1 : currentPosterTemplateIndex - 1;
    }

    function next() {
        currentPosterTemplateIndex = currentPosterTemplateIndex + 1 >= templates.length ? 0 : currentPosterTemplateIndex + 1;
    }

    onMount(() => {
        // sync templates if necessary
    })
            ...
            <div class="relative">
                <div class="w-full object-cover mx-auto max-w-md">
                    <div id="posterContainer" bind:this={posterContainerNode}>
                        <svelte:component this="{currentPosterTemplate}"/>
                    </div>
                </div>
            </div>
        </div>
